    ...
    if reading.natal_chart:
        reading_dict["natal_chart"] = {
            "birth_data": {
                ...
                "time": reading.natal_chart.birth_data.time.isoformat()
                if reading.natal_chart.birth_data.time
                else None,
            ...
            "planetary_positions": {
                body: deg
                for body, deg in reading.natal_chart.planetary_positions.items()
            },
        }
...
def cache_ephemeris(
    d: date, lat: float, lon: float, house_system: HouseSystem, tz: str | None = None
) -> Chart:
